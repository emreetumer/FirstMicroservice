...
app.MapPost("/categories/create", async (CreateCagetoryDto request, ApplicationDbContext context, CancellationToken cancellationToken) =>
{
    bool isNameExists = await context.Categories.AnyAsync(c => c.Name == request.Name, cancellationToken);

    if (isNameExists)
    {
        return Results.BadRequest(new { Message = "Kategori var." });
    }
    ...
    return Results.Ok(new { Message = "Category baþarýyla oluþturuldu." });
});
